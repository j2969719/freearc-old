...
---- Процедура generateDecryption добавляет к записи алгоритма сжатия+шифрования ключи,         ----
----------------------------------------------------------------------------------------------------
module Encryption (generateEncryption, generateDecryption) where
...
generateDecryption compressor decryption_info  =  mapM addKey compressor   where
  ...
  addKey algorithm | not (compressionIs "encryption?" algorithm)
                               = return algorithm    -- non-encryption algorithm stays untouched
                   | otherwise = do
    ...
    let (dont_ask_passwords, mvar_passwords, keyfiles, ask_decryption_password) = decryption_info
    ...
    let checkPwd password = doCheck (keyfiles++[""])
          where
            doCheck []            = Nothing
            doCheck (keyfile:kfs) =
              -- Вычислим ключ расшифровки key и recheckCode, используемый для быстрой проверки правильности пароля
              let (key, recheckCode) = deriveKey algorithm (password++keyfile) salt numIterations (length checkCode)
              in if recheckCode==checkCode       -- Если верификация по checkCode успешна
                   then (Just key)               --   то возвращаем найденный ключ алгоритма шифрования
                   else doCheck kfs              --   иначе - пробуем оставшиеся keyfiles
    ...
    let findDecryptionKey (password:pwds) = do
          case (checkPwd password) of            -- Если верификация в checkPwd прошла успешно
            Just key -> return key               --   то возвращаем найденный ключ алгоритма шифрования
        ...
        findDecryptionKey [] = do   -- Сюда мы попадаем если ни один старый пароль не подошёл
          ...
          case (checkPwd password) of            -- Если верификация в checkPwd прошла успешно
            Just key -> return key               --   то возвращаем найденный ключ алгоритма шифрования
            Nothing  -> do print "Bad password"  --   иначе - запрашиваем другой пароль
                           findDecryptionKey []
    --
    key <- findDecryptionKey passwords
    pl  <- val passwords_list
    return (pl, algorithm++":k"++encode16 key)
