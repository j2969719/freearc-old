...
class MYFILE
{
  int handle;
public:
  MYFILE& open (FILENAME filename, MODE mode)    // Открывает файл для чтения или записи
  {
    handle = ::open (filename, mode==READ_MODE? O_RDONLY|O_BINARY : O_WRONLY|O_BINARY|O_CREAT|O_TRUNC, S_IREAD|S_IWRITE);
  ...
  void     seek    (FILESIZE pos)    {CHECK( _lseeki64 (handle, pos, SEEK_SET) == pos, ("\nERROR: file seek operation failed"));}       // Переходит на заданную позицию в файле
#else
  FILESIZE size    ()                {return filelength (handle);}
  void     seek    (FILESIZE pos)    {CHECK( lseek (handle, pos, SEEK_SET) == pos, ("\nERROR: file seek operation failed"));}
#endif

  FILESIZE tryRead (void *buf, FILESIZE size)   {int result = ::read (handle, buf, size); CHECK(result>=0, ("\nERROR: file read operation failed")); return result;}           // Возвращает кол-во прочитанных байт, которое может быть меньше запрошенного
  void     read    (void *buf, FILESIZE size)   {CHECK (tryRead (buf, size) == size, ("\nERROR: can't read %lu bytes", (unsigned long)size));}         // Возбуждает исключение, если не удалось прочесть указанное число байт
  void     write   (void *buf, FILESIZE size)   {CHECK (::write (handle, buf, size) == size, ("\nERROR: file write operation failed"));}

  MYFILE ()                               {handle=-1;}
  MYFILE (FILENAME filename, MODE mode)   {open (filename, mode);}
  ~MYFILE()                               {if (handle>=0) close();}
};
...
public:
    ...
    MEMORY_BUFFER& open (MYFILE &file, FILESIZE pos, FILESIZE size)
    {
      ...
      CHECK (buf, ("\nERROR: can't alloc %lu memory bytes", (unsigned long)(size+8)));
      file.seek (pos);
      file.read (buf, size);
      p=buf, bufend=p+size;
      return *this;
    ...
    MEMORY_BUFFER& openCompressedCheckCRC (COMPRESSOR compressor, FILESIZE origsize, MYFILE &file, FILESIZE pos, FILESIZE compsize, CRC right_crc)
    {
      ...
      int result = DecompressMem (compressor, buf, compsize, origbuf, origsize, NULL);
      CHECK (result!=FREEARC_ERRCODE_INVALID_COMPRESSOR, ("\nERROR: unsupported compression method \"%s\"", compressor));
      CHECK (result==origsize, ("\nERROR: archive structure corrupted (decompression of control block failed)"));
...
struct LOCAL_BLOCK_DESCRIPTOR : BLOCK
{
  ...
  LOCAL_BLOCK_DESCRIPTOR (MYFILE &arcfile, FILESIZE descr_pos)
  {
    ...
    buffer.read4 (&crc );
    CHECK (sign==aSIGNATURE && origsize>0 && compsize>0 && compsize<=descr_pos, ("\nERROR: archive structure corrupted (strange descriptor)"));
...
FILESIZE FindFooterDescriptor (MYFILE &arcfile)
{
  ...
  for (char *ptr=buf+size-sizeof(uint32); ; ptr--) {
    ...
    CHECK (ptr>buf, ("\nERROR: this is not FreeArc archive or this archive is corrupt"));   // Сигнатура не найдена в последних MAX_FOOTER_DESCRIPTOR_SIZE байтах архива
  }
}
...
void ARCHIVE::read_structure()
{
  ...
  int cmtlen;  buffer.read (&cmtlen);            // Комментарий кодируется как массив символов с явно заданной длиной
...
DIRECTORY_BLOCK::DIRECTORY_BLOCK (ARCHIVE &arc, BLOCK &block_info) : arcfile (arc.arcfile)
{
  ...
  CHECK (block_info.type == DIR_BLOCK, ("\nINTERNAL ERROR: must be dir block"));
