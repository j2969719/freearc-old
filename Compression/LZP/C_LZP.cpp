...
#define LZP_INIT(HashSize,Pattern)                                               \
    ...
    BYTE* p, * InEnd=In+Size, * OutStart=Out, ** HTable = new BYTE*[HashSize];   \
...
{
    BYTE* OutEnd=Out+Size;   if (Size<32)  return -1;
...
int lzp_compress (MemSize BlockSize, int MinCompression, int MinMatchLen, int HashSizeLog, int Barrier, int SmallestLen, CALLBACK_FUNC *callback, VOID_FUNC *auxdata)
{
    ...
    while ( (x = callback ("read", (In = (BYTE*) malloc(BlockSize+2)), BlockSize, auxdata)) > 0 )
    {
        int InSize=x, OutSize;     // количество байт во входном и выходном буфере, соответственно
        In = (BYTE*) realloc(In,InSize);
        Out = (BYTE*) malloc(InSize+2);
...
// Функция распаковки
int LZP_METHOD::decompress (CALLBACK_FUNC *callback, VOID_FUNC *auxdata)
{
...
// Функция упаковки
int LZP_METHOD::compress (CALLBACK_FUNC *callback, VOID_FUNC *auxdata)
{
