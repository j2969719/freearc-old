                                   ...
                                   bool   eos)
{
  ...
  _matchFinderCycles = matchFinderCycles;

  int m = matchFinder; // FindMatchFinder(matchFinder);
  if (m < 0)
    return E_INVALIDARG;
  if (_matchFinder && m != _matchFinderIndex)
  {
    _dictionarySizePrev = UInt32(-1);
    _matchFinder.Release();
  }
  ...
  if (posStateBits > (UInt32)NLength::kNumPosStatesBitsEncodingMax)
    return E_INVALIDARG;
  _posStateBits = posStateBits;
  _posStateMask = (1 << _posStateBits) - 1;

  if (litPosBits > (UInt32)kNumLitPosStatesBitsEncodingMax)
    return E_INVALIDARG;
  _numLiteralPosStateBits = litPosBits;

  if (litContextBits > (UInt32)kNumLitContextBitsMax)
    return E_INVALIDARG;
  _numLiteralContextBits = litContextBits;

  SetWriteEndMarkerMode(eos);
