...
{
  CBCJ_x86_Encoder c; c.Init();                  // кодер для BCJ-X86 препроцессора
  ...
  int InSize;                                    // количество прочитанных байт
...
Error: delete Buf; return FREEARC_ERRCODE_GENERAL;  // произошла ошибка при чтении/записи
...
{
  CBCJ_x86_Decoder c; c.Init();                  // декодер для BCJ-X86 препроцессора
  BYTE* Buf = (BYTE*)malloc(LARGE_BUFFER_SIZE);  // место для данных
  if (Buf==NULL)   return FREEARC_ERRCODE_NOT_ENOUGH_MEMORY;
  int RemainderSize=0;                           // остаток данных с предыдущего раза
  int InSize;                                    // количество прочитанных байт
